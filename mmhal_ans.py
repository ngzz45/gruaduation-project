...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpt-model', type=str, default='gpt-4-0125-preview')
    args = parser.parse_args()

    openai.api_key = API_KEY

    # prepare evaluation file
    args.evaluation = args.response.replace('/mmhal_eval/', '/mmhal_ans/')
    if not os.path.exists(os.path.dirname(args.evaluation)):
    ...
    # append scores, hallucination, and scores_each to evaluation file
    if args.evaluation is not None:
        with open(args.evaluation, 'a') as f:
            json.dump({'scores': scores, 'hallucination': hallucination, 'scores_each': scores_each}, f, indent=2)
            f.write('\n')
            # save printed average score and hallucination rate, and average score for each question type to the same evaluation file
            f.write('Average score: {:.2f}\n'.format(sum(scores) / len(scores)))
            f.write('Hallucination rate: {:.2f}\n'.format(sum(hallucination) / len(hallucination)))
            f.write('Average score for each question type: {}\n'.format(','.join([str(round(sum(scores_each[i]) / len(scores_each[i]), 2)) for i in range(8)])))
            f.write('\n')